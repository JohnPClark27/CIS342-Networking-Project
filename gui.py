...
from PySide6.QtWidgets import (QApplication, QMainWindow, QComboBox, QToolBar, QWidget, QLabel, QSlider, QGridLayout,
                               QHBoxLayout, QPushButton, QSpinBox, QFileDialog, QTextEdit, QLineEdit, QSizePolicy)
...
class UIBuilder:
    ...
    def build_central_area(self):
        ...
        for w in [self.rec_ip_label, self.rec_ip_input, self.rec_port_spacer, QLabel("Port Number:"),
                  self.receiver_port_input]:
            right_port_layout.addWidget(w, alignment=Qt.AlignmentFlag.AlignTop)

        right_port_layout.addStretch()

        self.ip_widgets = [self.ip_label, self.ip_input, self.port_spacer, self.rec_ip_label, self.rec_ip_input,
                           self.rec_port_spacer]
...
class MainWindow(QMainWindow):
    ...
    def toggle_ip_fields(self, protocol_name):
        self.reset_logs()
        self.write_log(f"~ SYS: Protocol switched to {protocol_name}", "left", "info")
    ...
    def simulate_send(self):
        ...
        if not self.selected_file_path:
            self.write_log("~ SYS: ERROR: No file selected.", "left", "error")
            return
